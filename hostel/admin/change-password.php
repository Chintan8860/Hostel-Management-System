...
$udate=date('Y-m-d');
	...
	if($row_cnt>0)
	{
		$conn="update admin set password=?,updation_date=?  where id=?";
$chngpwd1 = $con->prepare($conn);
$chngpwd1->bind_param('ssi',$np,$udate,$ai);
  $chngpwd1->execute();
		$_REQUEST['msg']="Password Changed Successfully !!";
	}
	else
	{
		$_REQUEST['msg']="Old Password not match !!";
	}	
...
{ ?>
											...
											<p style="color: red"><?php echo "<script>window.open('dashboard.php',_self);</script>" ?></p>
											<?php } ?>
